...
public class NhapHangDAO {
  ...
  public static int  Insert(NhapHangDTO nhapHangDTO){
      try {
            Connection cons = DBConnect.getConnection();
            String sql = "INSERT INTO nhaphang(mancc, manv, mapn, tongtien, ngaynhap, ghichu) "
                    + "VALUES ( ?, ?, ?, ?, ?, ?)";
  ...
  public static int Update(NhapHangDTO nhapHangDTO){
        try {
            Connection cons = DBConnect.getConnection();
            String sql="UPDATE nhaphang SET mancc=?,manv=?,tongtien=?,ngaynhap=?,ghichu=? WHERE mapn=?";
            ...
            ps.setString(6, nhapHangDTO.getMapn());
